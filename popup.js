        ...
        target: { tabId: tabs[0].id },
          ...
          try {
            ...
            if (selectedOption === 'exclude-visitor-id' && Array.isArray(json.results) && json.results.length === 0) {
              const fallback = `https://app.pendo.io/api/s/${location.pathname.split('/s/')[1].split('/')[0]}/blacklist/type/visitorIdGlob?limit=1000&prefix=`;
            ...
            if (selectedOption === 'exclude-account-id' && Array.isArray(json.results) && json.results.length === 0) {
              const fallback2 = `https://app.pendo.io/api/s/${location.pathname.split('/s/')[1].split('/')[0]}/blacklist/type/accountIdGlob?limit=1000&prefix=`;
